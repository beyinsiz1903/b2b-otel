...
const AvailabilityPage = () => {
  const [form, setForm] = React.useState({
    ...
    capacity_label: "2+1", pax: 2,
    date_start: "", date_end: "", nights: 1,
    price_min: 0, price_max: 0,
    availability_status: "available",
    image_urls_raw: "", features_raw: "", notes: "",
  });
  const [mine, setMine] = React.useState([]);
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState("");
  const [success, setSuccess] = React.useState("");
  const [editModal, setEditModal] = React.useState(null); // listing to edit
  const [deleteConfirm, setDeleteConfirm] = React.useState(null);
  const [tab, setTab] = React.useState("create"); // "create" | "mine"

  const onChange = (e) => setForm({ ...form, [e.target.name]: e.target.value });

  const loadMine = async () => {
    try {
      const res = await axios.get("/listings/mine");
      setMine(res.data);
    } catch {
      setMine([]);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("/listings", {
        ...
        nights: Number(form.nights), price_min: price, price_max: price,
        availability_status: form.availability_status,
        image_urls, features, notes: form.notes,
      });
      setSuccess("✅ Kapasite ilanı yayınlandı!");
      setForm({ ...form, micro_location: "", date_start: "", date_end: "", notes: "", image_urls_raw: "", features_raw: "" });
      await loadMine();
      setTab("mine");
    } catch (err) {
    ...
    <Layout>
      ...
      {tab === "create" && (
        <>
          <div className="info-banner">
            ...
            Otel adı ve iletişim bilgisi eşleşme olmadan açılmaz. Lütfen görsellerde isim/logo kullanmayın.
          </div>
          <form className="availability-form" onSubmit={handleSubmit} data-testid="availability-form">
            ...
            <label className="field">
              <span>Özellikler (virgülle ayır veya hızlı ekle)</span>
              <textarea
                name="features_raw"
              ...
              <div style={{ marginTop: "0.5rem" }}>
                {FEATURES_LIST.map((chip) => {
                  const selected = form.features_raw.split(",").map((s) => s.trim()).includes(chip);
                  return (
                    <button
                      key={chip}
                      type="button"
        ...
        <>
          {mine.length === 0 ? (
            <div className="empty-state">
              ...
              <div className="empty-state-sub">İlk kapasite ilanınızı oluşturun.</div>
              ...
              {mine.map((l) => {
                ...
                return (
                  <div key={l.id} className="own-listing-card" style={{ opacity: expired ? 0.7 : 1 }}>
                    {l.image_urls && l.image_urls.length > 0 ? (
                      <div className="listing-image-wrapper">
                        <img src={l.image_urls[0]} alt="Kapasite" className="listing-image" />
                      </div>
                    ) : (
                      <div className="listing-image-wrapper" style={{ display: "flex", alignItems: "center", justifyContent: "center", color: "#9ca3af", fontSize: "2rem" }}>
                        🏨
                    ...
                    <div className="listing-card-inner">
                      ...
                      <div className="listing-body">
                        <div style={{ fontWeight: 600, color: "#1a3a2a" }}>{l.concept}</div>
                        <div>👥 {l.capacity_label} · {l.pax} kişi</div>
                    ...
                    <div className="own-listing-actions">
                      <button
                        ...
                        title={l.is_locked ? "Kilitli ilan düzenlenemez" : "Düzenle"}
                      >
                        ✏️ Düzenle
                      </button>
                      <button
                        ...
                        disabled={l.is_locked}
                        title={l.is_locked ? "Kilitli ilan silinemez" : "Sil"}
                      >
                        🗑️ Sil
                      </button>
                    </div>
      ...
      {editModal && (
        <EditListingModal
          ...
          onSaved={async () => { setEditModal(null); await loadMine(); }}
        />
      )}
      ...
      <ConfirmDialog
        open={!!deleteConfirm}
        title="İlanı Sil"
        message={`"${deleteConfirm?.concept}" ilanını kalıcı olarak silmek istediğinize emin misiniz?`}
