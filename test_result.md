...
agent_communication:
  - agent: "main"
    message: "V6 COMPREHENSIVE IMPROVEMENTS: 1) Security: JWT secret key in .env, password validation (min 8 chars, uppercase, lowercase, digit), input sanitization (XSS protection). 2) Pagination: All list endpoints (listings, mine, outgoing, incoming, matches, payments, invoices) now support skip/limit params with X-Total-Count header. 3) Full-text search: /listings?search=keyword. 4) PDF Invoice Export: GET /invoices/{id}/pdf. 5) Admin Activity Logs: GET /admin/activity-logs. 6) Frontend: Error Boundary (catches crashes), 404 NotFound page, Dark Mode toggle (persisted in localStorage), Skeleton loader component, Pagination component, improved mobile responsive CSS, print styles. 7) Admin panel: new 'Aktivite Log' tab. 8) Invoices: PDF download button. Login: admin@test.com / Admin123. Test all new features."
  - agent: "testing"
