...
agent_communication:
  - agent: "main"
    message: "V6 FRONTEND TESTING REQUEST: Test all new V6 frontend features. Login: admin@test.com / Admin123. Test: 1) DARK MODE: After login, click the theme toggle button (🌙/☀️) in top-right header next to notification bell. Verify background changes to dark, cards become dark, sidebar becomes dark. Click again to switch back to light. 2) 404 PAGE: Navigate to /nonexistent-page, verify '404 Sayfa Bulunamadı' page appears with 'Ana Sayfa' and 'Geri Dön' buttons. 3) ERROR BOUNDARY: Verify the app wraps in ErrorBoundary (just confirm app loads without crashing). 4) INVOICES PDF: Navigate to /invoices, if any invoice exists check for '📄 PDF' button next to 'Detay' button. 5) ADMIN ACTIVITY LOGS: Navigate to /admin, look for '📜 Aktivite Log' tab, click it and verify activity log table appears with columns Tarih, Kullanıcı, İşlem, Nesne, Detay. 6) MOBILE RESPONSIVE: Test at 480px viewport width - verify sidebar collapses, cards stack vertically. 7) SKELETON/PAGINATION components exist in code (visual components). 8) Overall navigation still works: Dashboard, Kapasiteler, Kendi Kapasitem, Envanter, Talepler, Eşleşmeler, Ödemeler, Faturalar, Abonelik, Fiyatlama, Raporlar, Profilim, Admin Panel, Performans."
  - agent: "testing"
